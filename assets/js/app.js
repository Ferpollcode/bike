...
function buildReceiptPdf(receipt) {
  ...
  text(state.settings.bizFooter || "", 205, 38, 9);
...
function bindEvents() {
  ...
  $("#receiptsList").addEventListener("click", (event) => {
    ...
    if (waId) {
      ...
      if (receipt) openWhatsapp(receipt);
    }
  });
  ...
  $("#whatsappReceipt").addEventListener("click", () => currentReceipt && openWhatsapp(currentReceipt));
